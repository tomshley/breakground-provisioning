...
TOMSHLEY_BREAKGROUND_BUILD_VERSION="$(cat "${CI_PROJECT_DIR}/VERSION")"
export TOMSHLEY_BREAKGROUND_BUILD_VERSION
TOMSHLEY_BREAKGROUND_BUILD_VERSION_NEXT="$(echo "$TOMSHLEY_BREAKGROUND_BUILD_VERSION" | gawk 'BEGIN { FS="." } { $3++;  if ($3 > 99) { $3=0; $2++; if ($2 > 99) { $2=0; $1++ } } } { printf "v%d.%d.%d\n", $1, $2, $3 }')"
...
#if [ -n "${CI_COMMIT_TAG-}" ]; then export TOMSHLEY_DOCKERS_BUILD_REF2="${CI_COMMIT_REF_SLUG}"; elif [ -n "${CI_COMMIT_REF_SLUG-}" ]; then export TOMSHLEY_DOCKERS_BUILD_REF2=${CI_COMMIT_REF_SLUG}-${CI_COMMIT_SHORT_SHA}; fi
#echo "CI_COMMIT_TAG222=${CI_COMMIT_TAG}"
#echo "TOMSHLEY_DOCKERS_BUILD_REF2222=${TOMSHLEY_DOCKERS_BUILD_REF2}"
