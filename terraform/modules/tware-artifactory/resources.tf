...
resource "artifactory_group" "hexagonal-deployers-group" {
  ...
  admin_privileges = false
  users_names      = [artifactory_user.hexagonal-deployer.id]
}
...
resource "artifactory_permission_target" "hexagonal-reader-perms" {
  ...
  repo {
    ...
    actions {
      groups {
        name        = artifactory_group.hexagonal-readers-group.name
        permissions = ["read"]
      }
    }
  }
...
resource "artifactory_permission_target" "hexagonal-deployer-perms" {
  ...
  repo {
    ...
    actions {
      groups {
        name        = artifactory_group.hexagonal-deployers-group.name
...
resource "artifactory_access_token" "hexagonal-reader-key" {
  username          = artifactory_user.hexagonal-reader.name
  end_date_relative = "2400h30m"
}

resource "artifactory_access_token" "hexagonal-deployer-key" {
  username          = artifactory_user.hexagonal-deployer.name
  end_date_relative = "2400h30m"
}
